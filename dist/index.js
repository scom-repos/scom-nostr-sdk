...
define("@scom/scom-social-sdk/utils/hashes/_assert.ts", ["require", "exports"], function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.output = exports.exists = exports.hash = exports.bytes = exports.bool = exports.number = void 0;
    ///<amd-module name='@scom/scom-social-sdk/utils/hashes/_assert.ts'/> 
    // adopted from https://github.com/paulmillr/noble-hashes
    function number(n) {
        if (!Number.isSafeInteger(n) || n < 0)
            throw new Error(`Wrong positive integer: ${n}`);
...
///<amd-module name='@scom/scom-social-sdk/utils/hashes/utils.ts'/> 
// adopted from https://github.com/paulmillr/noble-hashes
/*! noble-hashes - MIT License (c) 2022 Paul Miller (paulmillr.com) */
define("@scom/scom-social-sdk/utils/hashes/utils.ts", ["require", "exports"], function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.randomBytes = exports.wrapConstructor = exports.Hash = exports.concatBytes = exports.toBytes = exports.utf8ToBytes = exports.hexToBytes = exports.bytesToHex = exports.rotr = exports.createView = void 0;
    const u8a = (a) => a instanceof Uint8Array;
...
define("@scom/scom-social-sdk/utils/hashes/_sha2.ts", ["require", "exports", "@scom/scom-social-sdk/utils/hashes/_assert.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts"], function (require, exports, _assert_3, utils_14) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.SHA2 = void 0;
    // Polyfill for Safari 14
    ...
    // Base SHA2 class (RFC 6234)
    class SHA2 extends utils_14.Hash {
        constructor(blockLen, outputLen, padOffset, isLE) {
            super();
            this.blockLen = blockLen;
            this.outputLen = outputLen;
            ...
            this.destroyed = false;
            this.buffer = new Uint8Array(blockLen);
            this.view = (0, utils_14.createView)(this.buffer);
        }
        update(data) {
            (0, _assert_3.exists)(this);
            const { view, buffer, blockLen } = this;
            data = (0, utils_14.toBytes)(data);
            const len = data.length;
            for (let pos = 0; pos < len;) {
                const take = Math.min(blockLen - this.pos, len - pos);
                // Fast path: we have at least one block in input, cast it to view and process
                if (take === blockLen) {
                    const dataView = (0, utils_14.createView)(data);
                    for (; blockLen <= len - pos; pos += blockLen)
                        this.process(dataView, pos);
                    continue;
                }
        ...
        digestInto(out) {
            ...
            setBigUint64(view, blockLen - 8, BigInt(this.length * 8), isLE);
            this.process(view, 0);
            const oview = (0, utils_14.createView)(out);
            const len = this.outputLen;
            // NOTE: we do division by 4 later, which should be fused in single op with modulo by JIT
            if (len % 4)
                throw new Error('_sha2: outputLen should be aligned to 32bit');
...
define("@scom/scom-social-sdk/utils/hashes/sha256.ts", ["require", "exports", "@scom/scom-social-sdk/utils/hashes/_sha2.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts"], function (require, exports, _sha2_2, utils_15) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.sha224 = exports.sha256 = void 0;
    // SHA2-256 need to try 2^128 hashes to execute birthday attack.
    ...
    // Named this way because it matches specification.
    const SHA256_W = /* @__PURE__ */ new Uint32Array(64);
    class SHA256 extends _sha2_2.SHA2 {
        constructor() {
            super(64, 32, 8, false);
            // We cannot use array here since array allows indexing by variable
            // which means optimizer/compiler cannot use registers.
        ...
        process(view, offset) {
            ...
            for (let i = 16; i < 64; i++) {
                const W15 = SHA256_W[i - 15];
                const W2 = SHA256_W[i - 2];
                const s0 = (0, utils_15.rotr)(W15, 7) ^ (0, utils_15.rotr)(W15, 18) ^ (W15 >>> 3);
                const s1 = (0, utils_15.rotr)(W2, 17) ^ (0, utils_15.rotr)(W2, 19) ^ (W2 >>> 10);
                SHA256_W[i] = (s1 + SHA256_W[i - 7] + s0 + SHA256_W[i - 16]) | 0;
            }
            // Compression function main loop, 64 rounds
            let { A, B, C, D, E, F, G, H } = this;
            for (let i = 0; i < 64; i++) {
                const sigma1 = (0, utils_15.rotr)(E, 6) ^ (0, utils_15.rotr)(E, 11) ^ (0, utils_15.rotr)(E, 25);
                const T1 = (H + sigma1 + Chi(E, F, G) + SHA256_K[i] + SHA256_W[i]) | 0;
                const sigma0 = (0, utils_15.rotr)(A, 2) ^ (0, utils_15.rotr)(A, 13) ^ (0, utils_15.rotr)(A, 22);
                const T2 = (sigma0 + Maj(A, B, C)) | 0;
                H = G;
                G = F;
                F = E;
     ...
     * @param message - data that would be hashed
     */
    exports.sha256 = (0, utils_15.wrapConstructor)(() => new SHA256());
    exports.sha224 = (0, utils_15.wrapConstructor)(() => new SHA224());
});
define("@scom/scom-social-sdk/utils/curves/abstract/utils.ts", ["require", "exports"], function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.validateObject = exports.createHmacDrbg = exports.bitMask = exports.bitSet = exports.bitGet = exports.bitLen = exports.utf8ToBytes = exports.equalBytes = exports.concatBytes = exports.ensureBytes = exports.numberToVarBytesBE = exports.numberToBytesLE = exports.numberToBytesBE = exports.bytesToNumberLE = exports.bytesToNumberBE = exports.hexToBytes = exports.hexToNumber = exports.numberToHexUnpadded = exports.bytesToHex = void 0;
    ///<amd-module name='@scom/scom-social-sdk/utils/curves/abstract/utils.ts'/> 
    // adopted from https://github.com/paulmillr/noble-curves
    /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
    // 100 lines of code in the file are duplicated from noble-hashes (utils).
    // This is OK: `abstract` directory does not use noble-hashes.
...
// const z3 = validateObject(o, { test: 'boolean', z: 'bug' });
// const z4 = validateObject(o, { a: 'boolean', z: 'bug' });
define("@scom/scom-social-sdk/utils/curves/abstract/modular.ts", ["require", "exports", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts"], function (require, exports, utils_16) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.mapHashToField = exports.getMinHashLength = exports.getFieldBytesLength = exports.hashToPrivateScalar = exports.FpSqrtEven = exports.FpSqrtOdd = exports.Field = exports.nLength = exports.FpIsSquare = exports.FpDiv = exports.FpInvertBatch = exports.FpPow = exports.validateField = exports.isNegativeLE = exports.FpSqrt = exports.tonelliShanks = exports.invert = exports.pow2 = exports.pow = exports.mod = void 0;
    // prettier-ignore
    ...
    function validateField(field) {
        ...
        return (0, utils_16.validateObject)(field, opts);
    }
    exports.validateField = validateField;
    // Generic field functions
    /**
    ...
    function Field(ORDER, bitLen, isLE = false, redef = {}) {
        ...
        const f = Object.freeze({
            ...
            // We don't have const-time bigints anyway, so probably will be not very useful
            cmov: (a, b, c) => (c ? b : a),
            toBytes: (num) => (isLE ? (0, utils_16.numberToBytesLE)(num, BYTES) : (0, utils_16.numberToBytesBE)(num, BYTES)),
            fromBytes: (bytes) => {
                if (bytes.length !== BYTES)
                    throw new Error(`Fp.fromBytes: expected ${BYTES}, got ${bytes.length}`);
                return isLE ? (0, utils_16.bytesToNumberLE)(bytes) : (0, utils_16.bytesToNumberBE)(bytes);
            },
        });
        return Object.freeze(f);
    }
     ...
     */
    function hashToPrivateScalar(hash, groupOrder, isLE = false) {
        hash = (0, utils_16.ensureBytes)('privateHash', hash);
        const hashLen = hash.length;
        const minLen = nLength(groupOrder).nByteLength + 8;
        if (minLen < 24 || hashLen < minLen || hashLen > 1024)
            throw new Error(`hashToPrivateScalar: expected ${minLen}-1024 bytes of input, got ${hashLen}`);
        const num = isLE ? (0, utils_16.bytesToNumberLE)(hash) : (0, utils_16.bytesToNumberBE)(hash);
        return mod(num, groupOrder - _1n) + _1n;
    }
    exports.hashToPrivateScalar = hashToPrivateScalar;
    /**
    ...
    function mapHashToField(key, fieldOrder, isLE = false) {
        ...
        if (len < 16 || len < minLen || len > 1024)
            throw new Error(`expected ${minLen}-1024 bytes of input, got ${len}`);
        const num = isLE ? (0, utils_16.bytesToNumberBE)(key) : (0, utils_16.bytesToNumberLE)(key);
        // `mod(x, 11)` can sometimes produce 0. `mod(x, 10) + 1` is the same, but no 0
        const reduced = mod(num, fieldOrder - _1n) + _1n;
        return isLE ? (0, utils_16.numberToBytesLE)(reduced, fieldLen) : (0, utils_16.numberToBytesBE)(reduced, fieldLen);
    }
    exports.mapHashToField = mapHashToField;
});
define("@scom/scom-social-sdk/utils/curves/abstract/curve.ts", ["require", "exports", "@scom/scom-social-sdk/utils/curves/abstract/modular.ts", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts"], function (require, exports, modular_4, utils_17) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.validateBasic = exports.wNAF = void 0;
    const _0n = BigInt(0);
    ...
    exports.wNAF = wNAF;
    function validateBasic(curve) {
        (0, modular_4.validateField)(curve.Fp);
        (0, utils_17.validateObject)(curve, {
            n: 'bigint',
            h: 'bigint',
            Gx: 'field',
            Gy: 'field',
        ...
        // Set defaults
        return Object.freeze({
            ...(0, modular_4.nLength)(curve.n, curve.nBitLength),
            ...curve,
            ...{ p: curve.Fp.ORDER },
        });
    }
...
define("@scom/scom-social-sdk/utils/curves/abstract/weierstrass.ts", ["require", "exports", "@scom/scom-social-sdk/utils/curves/abstract/modular.ts", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts", "@scom/scom-social-sdk/utils/curves/abstract/curve.ts"], function (require, exports, mod, ut, utils_18, curve_2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.mapToCurveSimpleSWU = exports.SWUFpSqrtRatio = exports.weierstrass = exports.weierstrassPoints = exports.DER = void 0;
    function validatePointOpts(curve) {
        const opts = (0, curve_2.validateBasic)(curve);
        ut.validateObject(opts, {
            a: 'field',
            b: 'field',
        }, {
    ...
    function weierstrassPoints(opts) {
        ...
        function normPrivateKeyToScalar(key) {
            ...
            try {
                num =
                    typeof key === 'bigint'
                        ? key
                        : ut.bytesToNumberBE((0, utils_18.ensureBytes)('private key', key, nByteLength));
            }
            catch (error) {
                throw new Error(`private key must be ${nByteLength} bytes, hex or bigint, not ${typeof key}`);
            }
        ...
        class Point {
             ...
             */
            static fromHex(hex) {
                const P = Point.fromAffine(fromBytes((0, utils_18.ensureBytes)('pointHex', hex)));
                P.assertValidity();
                return P;
            }
            // Multiplies generator point by privateKey.
        ...
        Point.ZERO = new Point(Fp.ZERO, Fp.ONE, Fp.ZERO);
        const _bits = CURVE.nBitLength;
        const wnaf = (0, curve_2.wNAF)(Point, CURVE.endo ? Math.ceil(_bits / 2) : _bits);
        // Validate if generator point is on curve
        return {
            CURVE,
            ProjectivePoint: Point,
    ...
    function weierstrass(curveDef) {
        ...
        class Signature {
            ...
            static fromCompact(hex) {
                const l = CURVE.nByteLength;
                hex = (0, utils_18.ensureBytes)('compactSignature', hex, l * 2);
                return new Signature(slcNum(hex, 0, l), slcNum(hex, l, 2 * l));
            }
            // DER encoded ECDSA signature
            // https://bitcoin.stackexchange.com/questions/57644/what-are-the-parts-of-a-bitcoin-transaction-input-script
            static fromDER(hex) {
                const { r, s } = exports.DER.toSig((0, utils_18.ensureBytes)('DER', hex));
                return new Signature(r, s);
            }
            assertValidity() {
                // can use assertGE here
            ...
            recoverPublicKey(msgHash) {
                const { r, s, recovery: rec } = this;
                const h = bits2int_modN((0, utils_18.ensureBytes)('msgHash', msgHash)); // Truncate hash
                if (rec == null || ![0, 1, 2, 3].includes(rec))
                    throw new Error('recovery id invalid');
                const radj = rec === 2 || rec === 3 ? r + CURVE.n : r;
                if (radj >= Fp.ORDER)
        ...
        function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
            ...
            if (lowS == null)
                lowS = true; // RFC6979 3.2: we skip step A, because we already provide hash
            msgHash = (0, utils_18.ensureBytes)('msgHash', msgHash);
            if (prehash)
                msgHash = (0, utils_18.ensureBytes)('prehashed msgHash', hash(msgHash));
            // We can't later call bits2octets, since nested bits2int is broken for curves
            // with nBitLength % 8 !== 0. Because of that, we unwrap it here as int2octets call.
            // const bits2octets = (bits) => int2octets(bits2int_modN(bits))
            const h1int = bits2int_modN(msgHash);
            ...
            if (ent != null) {
                // K = HMAC_K(V || 0x00 || int2octets(x) || bits2octets(h1) || k')
                const e = ent === true ? randomBytes(Fp.BYTES) : ent; // generate random bytes OR pass as-is
                seedArgs.push((0, utils_18.ensureBytes)('extraEntropy', e)); // check for being bytes
            }
            const seed = ut.concatBytes(...seedArgs); // Step D of RFC6979 3.2
            const m = h1int; // NOTE: no need to call bits2int second time here, it is inside truncateHash!
            // Converts signature params into point w r/s, checks result for validity.
        ...
        function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
            const sg = signature;
            msgHash = (0, utils_18.ensureBytes)('msgHash', msgHash);
            publicKey = (0, utils_18.ensureBytes)('publicKey', publicKey);
            if ('strict' in opts)
                throw new Error('options.strict was renamed to lowS');
            const { lowS, prehash } = opts;
            let _sig = undefined;
...
define("@scom/scom-social-sdk/utils/curves/abstract/hash-to-curve.ts", ["require", "exports", "@scom/scom-social-sdk/utils/curves/abstract/modular.ts", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts"], function (require, exports, modular_5, utils_19) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.createHasher = exports.isogenyMap = exports.hash_to_field = exports.expand_message_xof = exports.expand_message_xmd = void 0;
    function validateDST(dst) {
        if (dst instanceof Uint8Array)
            return dst;
        if (typeof dst === 'string')
            return (0, utils_19.utf8ToBytes)(dst);
        throw new Error('DST must be Uint8Array or string');
    }
    // Octet Stream to Integer. "spec" implementation of os2ip is 2.5x slower vs bytesToNumberBE.
    const os2ip = utils_19.bytesToNumberBE;
    // Integer to Octet Stream (numberToBytesBE)
    function i2osp(value, length) {
        if (value < 0 || value >= 1 << (8 * length)) {
            throw new Error(`bad I2OSP call: value=${value} length=${length}`);
    ...
    function expand_message_xmd(msg, DST, lenInBytes, H) {
        ...
        // https://www.rfc-editor.org/rfc/rfc9380#section-5.3.3
        if (DST.length > 255)
            DST = H((0, utils_19.concatBytes)((0, utils_19.utf8ToBytes)('H2C-OVERSIZE-DST-'), DST));
        const { outputLen: b_in_bytes, blockLen: r_in_bytes } = H;
        const ell = Math.ceil(lenInBytes / b_in_bytes);
        if (ell > 255)
            throw new Error('Invalid xmd length');
        const DST_prime = (0, utils_19.concatBytes)(DST, i2osp(DST.length, 1));
        const Z_pad = i2osp(0, r_in_bytes);
        const l_i_b_str = i2osp(lenInBytes, 2); // len_in_bytes_str
        const b = new Array(ell);
        const b_0 = H((0, utils_19.concatBytes)(Z_pad, msg, l_i_b_str, i2osp(0, 1), DST_prime));
        b[0] = H((0, utils_19.concatBytes)(b_0, i2osp(1, 1), DST_prime));
        for (let i = 1; i <= ell; i++) {
            const args = [strxor(b_0, b[i - 1]), i2osp(i + 1, 1), DST_prime];
            b[i] = H((0, utils_19.concatBytes)(...args));
        }
        const pseudo_random_bytes = (0, utils_19.concatBytes)(...b);
        return pseudo_random_bytes.slice(0, lenInBytes);
    }
    exports.expand_message_xmd = expand_message_xmd;
    // Produces a uniformly random byte string using an extendable-output function (XOF) H.
    ...
    function expand_message_xof(msg, DST, lenInBytes, k, H) {
        ...
        if (DST.length > 255) {
            const dkLen = Math.ceil((2 * k) / 8);
            DST = H.create({ dkLen }).update((0, utils_19.utf8ToBytes)('H2C-OVERSIZE-DST-')).update(DST).digest();
        }
        if (lenInBytes > 65535 || DST.length > 255)
            throw new Error('expand_message_xof: invalid lenInBytes');
        return (H.create({ dkLen: lenInBytes })
     ...
     */
    function hash_to_field(msg, count, options) {
        (0, utils_19.validateObject)(options, {
            DST: 'stringOrUint8Array',
            p: 'bigint',
            m: 'isSafeInteger',
            k: 'isSafeInteger',
        ...
        for (let i = 0; i < count; i++) {
            ...
            for (let j = 0; j < m; j++) {
                const elm_offset = L * (j + i * m);
                const tv = prb.subarray(elm_offset, elm_offset + L);
                e[j] = (0, modular_5.mod)(os2ip(tv), p);
            }
            u[i] = e;
        }
        return u;
...
define("@scom/scom-social-sdk/utils/hashes/hmac.ts", ["require", "exports", "@scom/scom-social-sdk/utils/hashes/_assert.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts"], function (require, exports, _assert_4, utils_20) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.hmac = exports.HMAC = void 0;
    // HMAC (RFC 2104)
    class HMAC extends utils_20.Hash {
        constructor(hash, _key) {
            super();
            this.finished = false;
            this.destroyed = false;
            (0, _assert_4.hash)(hash);
            const key = (0, utils_20.toBytes)(_key);
            this.iHash = hash.create();
            if (typeof this.iHash.update !== 'function')
                throw new Error('Expected instance of class which extends utils.Hash');
            this.blockLen = this.iHash.blockLen;
        ...
        update(buf) {
            (0, _assert_4.exists)(this);
            this.iHash.update(buf);
            return this;
        }
        digestInto(out) {
            (0, _assert_4.exists)(this);
            (0, _assert_4.bytes)(out, this.outputLen);
            this.finished = true;
            this.iHash.digestInto(out);
            this.oHash.update(out);
            this.oHash.digestInto(out);
...
define("@scom/scom-social-sdk/utils/curves/_shortw_utils.ts", ["require", "exports", "@scom/scom-social-sdk/utils/hashes/hmac.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts", "@scom/scom-social-sdk/utils/curves/abstract/weierstrass.ts"], function (require, exports, hmac_2, utils_21, weierstrass_3) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.createCurve = exports.getHash = void 0;
    // connects noble-curves to noble-hashes
    function getHash(hash) {
        return {
            hash,
            hmac: (key, ...msgs) => (0, hmac_2.hmac)(hash, key, (0, utils_21.concatBytes)(...msgs)),
            randomBytes: utils_21.randomBytes,
        };
    }
    exports.getHash = getHash;
    function createCurve(curveDef, defHash) {
        const create = (hash) => (0, weierstrass_3.weierstrass)({ ...curveDef, ...getHash(hash) });
        return Object.freeze({ ...create(defHash), create });
    }
    exports.createCurve = createCurve;
});
define("@scom/scom-social-sdk/utils/curves/secp256k1.ts", ["require", "exports", "@scom/scom-social-sdk/utils/hashes/sha256.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts", "@scom/scom-social-sdk/utils/curves/abstract/modular.ts", "@scom/scom-social-sdk/utils/curves/abstract/weierstrass.ts", "@scom/scom-social-sdk/utils/curves/abstract/utils.ts", "@scom/scom-social-sdk/utils/curves/abstract/hash-to-curve.ts", "@scom/scom-social-sdk/utils/curves/_shortw_utils.ts"], function (require, exports, sha256_3, utils_22, modular_6, weierstrass_4, utils_23, hash_to_curve_2, _shortw_utils_2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.schnorrGetExtPubKeyY = exports.encodeToCurve = exports.hashToCurve = exports.schnorr = exports.secp256k1 = void 0;
    const secp256k1P = BigInt('0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f');
    ...
    function sqrtMod(y) {
        ...
        const b2 = (y * y * y) % P; // x^3, 11
        const b3 = (b2 * b2 * y) % P; // x^7
        const b6 = ((0, modular_6.pow2)(b3, _3n, P) * b3) % P;
        const b9 = ((0, modular_6.pow2)(b6, _3n, P) * b3) % P;
        const b11 = ((0, modular_6.pow2)(b9, _2n, P) * b2) % P;
        const b22 = ((0, modular_6.pow2)(b11, _11n, P) * b11) % P;
        const b44 = ((0, modular_6.pow2)(b22, _22n, P) * b22) % P;
        const b88 = ((0, modular_6.pow2)(b44, _44n, P) * b44) % P;
        const b176 = ((0, modular_6.pow2)(b88, _88n, P) * b88) % P;
        const b220 = ((0, modular_6.pow2)(b176, _44n, P) * b44) % P;
        const b223 = ((0, modular_6.pow2)(b220, _3n, P) * b3) % P;
        const t1 = ((0, modular_6.pow2)(b223, _23n, P) * b22) % P;
        const t2 = ((0, modular_6.pow2)(t1, _6n, P) * b2) % P;
        const root = (0, modular_6.pow2)(t2, _2n, P);
        if (!Fp.eql(Fp.sqr(root), y))
            throw new Error('Cannot find square root');
        return root;
    }
    const Fp = (0, modular_6.Field)(secp256k1P, undefined, undefined, { sqrt: sqrtMod });
    exports.secp256k1 = (0, _shortw_utils_2.createCurve)({
        a: BigInt(0),
        b: BigInt(7),
        Fp,
        n: secp256k1N,
        ...
        endo: {
            ...
            splitScalar: (k) => {
                ...
                const c1 = divNearest(b2 * k, n);
                const c2 = divNearest(-b1 * k, n);
                let k1 = (0, modular_6.mod)(k - c1 * a1 - c2 * a2, n);
                let k2 = (0, modular_6.mod)(-c1 * b1 - c2 * b2, n);
                const k1neg = k1 > POW_2_128;
                const k2neg = k2 > POW_2_128;
                if (k1neg)
                    k1 = n - k1;
    ...
    function taggedHash(tag, ...messages) {
        let tagP = TAGGED_HASH_PREFIXES[tag];
        if (tagP === undefined) {
            const tagH = (0, sha256_3.sha256)(Uint8Array.from(tag, (c) => c.charCodeAt(0)));
            tagP = (0, utils_23.concatBytes)(tagH, tagH);
            TAGGED_HASH_PREFIXES[tag] = tagP;
        }
        return (0, sha256_3.sha256)((0, utils_23.concatBytes)(tagP, ...messages));
    }
    // ECDSA compact points are 33-byte. Schnorr is 32: we strip first byte 0x02 or 0x03
    const pointToBytes = (point) => point.toRawBytes(true).slice(1);
    const numTo32b = (n) => (0, utils_23.numberToBytesBE)(n, 32);
    const modP = (x) => (0, modular_6.mod)(x, secp256k1P);
    const modN = (x) => (0, modular_6.mod)(x, secp256k1N);
    const Point = exports.secp256k1.ProjectivePoint;
    const GmulAdd = (Q, a, b) => Point.BASE.multiplyAndAddUnsafe(Q, a, b);
    // Calculate point, scalar and bytes
    function schnorrGetExtPubKey(priv) {
     ...
     */
    function challenge(...args) {
        return modN((0, utils_23.bytesToNumberBE)(taggedHash('BIP0340/challenge', ...args)));
    }
    /**
     * Schnorr public key is just `x` coordinate of Point as per BIP340.
     */
     ...
     * auxRand is optional and is not the sole source of k generation: bad CSPRNG won't be dangerous.
     */
    function schnorrSign(message, privateKey, auxRand = (0, utils_22.randomBytes)(32)) {
        const m = (0, utils_23.ensureBytes)('message', message);
        const { bytes: px, scalar: d } = schnorrGetExtPubKey(privateKey); // checks for isWithinCurveOrder
        const a = (0, utils_23.ensureBytes)('auxRand', auxRand, 32); // Auxiliary random data a: a 32-byte array
        const t = numTo32b(d ^ (0, utils_23.bytesToNumberBE)(taggedHash('BIP0340/aux', a))); // Let t be the byte-wise xor of bytes(d) and hash/aux(a)
        const rand = taggedHash('BIP0340/nonce', t, px, m); // Let rand = hash/nonce(t || bytes(P) || m)
        const k_ = modN((0, utils_23.bytesToNumberBE)(rand)); // Let k' = int(rand) mod n
        if (k_ === _0n)
            throw new Error('sign failed: k is zero'); // Fail if k' = 0.
        const { bytes: rx, scalar: k } = schnorrGetExtPubKey(k_); // Let R = k'⋅G.
        const e = challenge(rx, px, m); // Let e = int(hash/challenge(bytes(R) || bytes(P) || m)) mod n.
     ...
     */
    function schnorrVerify(signature, message, publicKey) {
        const sig = (0, utils_23.ensureBytes)('signature', signature, 64);
        const m = (0, utils_23.ensureBytes)('message', message);
        const pub = (0, utils_23.ensureBytes)('publicKey', publicKey, 32);
        try {
            const P = lift_x((0, utils_23.bytesToNumberBE)(pub)); // P = lift_x(int(pk)); fail if that fails
            const r = (0, utils_23.bytesToNumberBE)(sig.subarray(0, 32)); // Let r = int(sig[0:32]); fail if r ≥ p.
            if (!fe(r))
                return false;
            const s = (0, utils_23.bytesToNumberBE)(sig.subarray(32, 64)); // Let s = int(sig[32:64]); fail if s ≥ n.
            if (!ge(s))
                return false;
            const e = challenge(numTo32b(r), pointToBytes(P), m); // int(challenge(bytes(r)||bytes(P)||m))%n
            const R = GmulAdd(P, s, modN(-e)); // R = s⋅G - e⋅P
    ...
    exports.schnorr = (() => ({
        ...
        utils: {
            ...
            lift_x,
            pointToBytes,
            numberToBytesBE: utils_23.numberToBytesBE,
            bytesToNumberBE: utils_23.bytesToNumberBE,
            taggedHash,
            mod: modular_6.mod,
        },
    }))();
    const isoMap = /* @__PURE__ */ (() => (0, hash_to_curve_2.isogenyMap)(Fp, [
        // xNum
        [
            '0x8e38e38e38e38e38e38e38e38e38e38e38e38e38e38e38e38e38e38daaaaa8c7',
            '0x7d3d4c80bc321d5b9f315cea7fd44c5d595d2fc0bf63b92dfff1044f17c6581',
    ...
    const mapSWU = /* @__PURE__ */ (() => (0, weierstrass_4.mapToCurveSimpleSWU)(Fp, {
        A: BigInt('0x3f8731abdd661adca08a5558f0f5d272e953d363cb6f0e5d405447c01a444533'),
        B: BigInt('1771'),
        Z: Fp.create(BigInt('-11')),
    }))();
    const htf = /* @__PURE__ */ (() => (0, hash_to_curve_2.createHasher)(exports.secp256k1.ProjectivePoint, (scalars) => {
        const { x, y } = mapSWU(Fp.create(scalars[0]));
        return isoMap(x, y);
    }, {
        DST: 'secp256k1_XMD:SHA-256_SSWU_RO_',
...
define("@scom/scom-social-sdk/utils/keys.ts", ["require", "exports", "@scom/scom-social-sdk/utils/curves/secp256k1.ts", "@scom/scom-social-sdk/utils/hashes/utils.ts"], function (require, exports, secp256k1_3, utils_24) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.decompressPublicKey = exports.getSharedSecret = exports.getPublicKeyY = exports.getPublicKey = exports.generatePrivateKey = void 0;
    function generatePrivateKey() {
        return (0, utils_24.bytesToHex)(secp256k1_3.schnorr.utils.randomPrivateKey());
    }
    exports.generatePrivateKey = generatePrivateKey;
    ;
    function getPublicKey(privateKey) {
        return (0, utils_24.bytesToHex)(secp256k1_3.schnorr.getPublicKey(privateKey));
    }
    exports.getPublicKey = getPublicKey;
    ;
    function getPublicKeyY(privateKey) {
        return (0, utils_24.bytesToHex)((0, secp256k1_3.schnorrGetExtPubKeyY)(privateKey));
    }
    exports.getPublicKeyY = getPublicKeyY;
    ;
    function getSharedSecret(privateKey, publicKey) {
        ...
        return (0, utils_24.bytesToHex)(secp256k1_3.secp256k1.getSharedSecret(privateKey, publicKey));
    }
    exports.getSharedSecret = getSharedSecret;
    ;
    function decompressPublicKey(publicKey) {
        const decompressedPublicKey = secp256k1_3.secp256k1.ProjectivePoint.fromHex(publicKey).toHex(false);
        return decompressedPublicKey;
    }
    exports.decompressPublicKey = decompressPublicKey;
    ;
});
define("@scom/scom-social-sdk/utils/crypto.ts", ["require", "exports", "@scom/scom-social-sdk/utils/keys.ts"], function (require, exports, Keys) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
define("@scom/scom-social-sdk/utils/index.ts", ["require", "exports", "@scom/scom-social-sdk/utils/interfaces.ts", "@scom/scom-social-sdk/utils/mqtt.ts", "@scom/scom-social-sdk/utils/crypto.ts"], function (require, exports, interfaces_1, mqtt_1, crypto_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
define("@scom/scom-social-sdk/managers/utilsManager.ts", ["require", "exports", "@ijstech/eth-wallet", "@scom/scom-social-sdk/core/index.ts", "@scom/scom-social-sdk/utils/interfaces.ts", "@scom/scom-signer"], function (require, exports, eth_wallet_1, index_2, interfaces_2, scom_signer_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    class SocialUtilsManager {
        ...
        static augmentWithAuthInfo(obj, privateKey) {
            if (!privateKey)
                return obj;
            const pubkey = index_2.Keys.getPublicKey(privateKey);
            let signature = scom_signer_1.Signer.getSignature({ pubkey: pubkey }, privateKey, { pubkey: 'string' });
            return {
                ...obj,
                auth: {
                    pubkey,
...
define("@scom/scom-social-sdk/managers/eventManagerRead.ts", ["require", "exports", "@scom/scom-social-sdk/core/index.ts", "@scom/scom-social-sdk/managers/utilsManager.ts"], function (require, exports, index_4, utilsManager_2) {
    ...
    class NostrEventManagerRead {
        ...
        async resetMessageCount(pubKey, sender) {
            ...
            event.sig = index_4.Event.getSignature(event, this._privateKey);
            const msg = {
...
define("@scom/scom-social-sdk/managers/eventManagerReadV2.ts", ["require", "exports", "@scom/scom-social-sdk/core/index.ts", "@scom/scom-social-sdk/managers/utilsManager.ts", "@scom/scom-social-sdk/managers/eventManagerRead.ts"], function (require, exports, index_5, utilsManager_3, eventManagerRead_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    class NostrEventManagerReadV2 extends eventManagerRead_1.NostrEventManagerRead {
        ...
        augmentWithAuthInfo(obj) {
            return utilsManager_3.SocialUtilsManager.augmentWithAuthInfo(obj, this._privateKey);
        }
        async fetchThreadCacheEvents(id, pubKey) {
            let decodedId = id.startsWith('note1') ? index_5.Nip19.decode(id).data : id;
        ...
        async fetchUserBookmarkedCommunities(pubKey, excludedCommunity) {
            ...
            for (let community of response.data) {
                if (excludedCommunity) {
                    const decodedPubkey = index_5.Nip19.decode(excludedCommunity.creatorId).data;
        ...
        async fetchCommunity(creatorId, communityId) {
            const decodedCreatorId = creatorId.startsWith('npub1') ? index_5.Nip19.decode(creatorId).data : creatorId;
            let msg = this.augmentWithAuthInfo({
                identifiers: [
                    {
        ...
        async fetchChannelMessages(channelId, since = 0, until = 0) {
            const decodedChannelId = channelId.startsWith('npub1') ? index_5.Nip19.decode(channelId).data : channelId;
            let msg = this.augmentWithAuthInfo({
        ...
        async fetchDirectMessages(pubKey, sender, since = 0, until = 0) {
            const decodedPubKey = pubKey.startsWith('npub1') ? index_5.Nip19.decode(pubKey).data : pubKey;
            const decodedSenderPubKey = sender.startsWith('npub1') ? index_5.Nip19.decode(sender).data : sender;
            const msg = this.augmentWithAuthInfo({
                receiver: decodedPubKey,
                sender: decodedSenderPubKey,
                limit: 20
...
define("@scom/scom-social-sdk/managers/index.ts", ["require", "exports", "@scom/scom-social-sdk/core/index.ts", "@scom/scom-social-sdk/utils/interfaces.ts", "@scom/scom-social-sdk/managers/communication.ts", "@scom/scom-social-sdk/utils/geohash.ts", "@scom/scom-social-sdk/utils/mqtt.ts", "@scom/scom-social-sdk/utils/lightningWallet.ts", "@scom/scom-social-sdk/managers/utilsManager.ts", "@scom/scom-social-sdk/managers/eventManagerWrite.ts", "@scom/scom-social-sdk/managers/eventManagerRead.ts", "@scom/scom-social-sdk/managers/eventManagerReadV2.ts", "@scom/scom-social-sdk/utils/crypto.ts"], function (require, exports, index_6, interfaces_4, communication_1, geohash_1, mqtt_2, lightningWallet_1, utilsManager_4, eventManagerWrite_1, eventManagerRead_2, eventManagerReadV2_1, crypto_2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    Object.defineProperty(exports, "SocialUtilsManager", { enumerable: true, get: function () { return utilsManager_4.SocialUtilsManager; } });
    ...
    Object.defineProperty(exports, "NostrEventManagerRead", { enumerable: true, get: function () { return eventManagerRead_2.NostrEventManagerRead; } });
    ...
    class SocialDataManager {
        ...
        async retrievePostPrivateKey(event, communityUri, communityPrivateKey) {
            let key = null;
            let postScpData = utilsManager_4.SocialUtilsManager.extractScpData(event, interfaces_4.ScpStandardId.CommunityPost);
            try {
                const postPrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(communityPrivateKey, event.pubkey, postScpData.encryptedKey);
                const messageContentStr = await utilsManager_4.SocialUtilsManager.decryptMessage(postPrivateKey, event.pubkey, event.content);
        ...
        async retrieveChannelMessagePrivateKey(event, channelId, communityPrivateKey) {
            let key = null;
            let messageScpData = utilsManager_4.SocialUtilsManager.extractScpData(event, interfaces_4.ScpStandardId.ChannelMessage);
            try {
                const messagePrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(communityPrivateKey, event.pubkey, messageScpData.encryptedKey);
                const messageContentStr = await utilsManager_4.SocialUtilsManager.decryptMessage(messagePrivateKey, event.pubkey, event.content);
                const messageContent = JSON.parse(messageContentStr);
        ...
        async retrieveCommunityPrivateKey(communityInfo, selfPrivateKey) {
            ...
            if (communityInfo.membershipType === interfaces_4.MembershipType.InviteOnly) {
                ...
                const pubkey = utilsManager_4.SocialUtilsManager.convertPrivateKeyToPubkey(selfPrivateKey);
                const encryptedKey = communityInfo.memberKeyMap?.[pubkey];
                if (encryptedKey) {
                    communityPrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(selfPrivateKey, creatorPubkey, encryptedKey);
                }
            }
            else if (communityInfo.membershipType === interfaces_4.MembershipType.NFTExclusive) {
                communityPrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(selfPrivateKey, communityInfo.scpData.gatekeeperPublicKey, communityInfo.scpData.encryptedKey);
            }
        ...
        async retrieveCommunityPostKeysByNoteEvents(options) {
            ...
            for (let communityInfo of noteCommunityMappings.communityInfoList) {
                if (options.pubKey === communityInfo.creatorId) {
                    let communityPrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(this._privateKey, communityInfo.scpData.gatekeeperPublicKey, communityInfo.scpData.encryptedKey);
                    if (communityPrivateKey) {
        ...
        createNoteEventMappings(events, parentAuthorsInfo = false) {
            ...
            for (let event of events) {
                if (event.kind === 0) {
                    ...
                    const noteEvent = utilsManager_4.SocialUtilsManager.parseContent(event.content);
                    ...
                    if (!event.content)
                        continue;
                    const originalNoteContent = utilsManager_4.SocialUtilsManager.parseContent(event.content);
                    notes.push({
        ...
        async fetchCommunityInfo(creatorId, communityId) {
            ...
            if (!communityEvent)
                return null;
            let communityInfo = utilsManager_4.SocialUtilsManager.extractCommunityInfo(communityEvent);
            //FIXME: not the best way to do this
            if (communityInfo.membershipType === interfaces_4.MembershipType.InviteOnly) {
                const keyEvent = await this._socialEventManagerRead.fetchGroupKeys(communityInfo.communityUri + ':keys');
                if (keyEvent) {
        ...
        async fetchThreadNotesInfo(focusedNoteId) {
            ...
            let scpData = utilsManager_4.SocialUtilsManager.extractScpData(focusedNote.eventData, interfaces_4.ScpStandardId.CommunityPost);
            if (scpData) {
        ...
        async createNoteCommunityMappings(notes) {
            ...
            for (let note of notes) {
                let scpData = utilsManager_4.SocialUtilsManager.extractScpData(note, interfaces_4.ScpStandardId.CommunityPost);
                if (scpData) {
            ...
            if (noteCommunityInfoList.length > 0) {
                ...
                for (let event of communityEvents) {
                    let communityInfo = utilsManager_4.SocialUtilsManager.extractCommunityInfo(event);
        ...
        async retrieveUserProfileDetail(pubKey) {
            ...
            for (let event of userContactEvents) {
                if (event.kind === 0) {
                    ...
                    let content = utilsManager_4.SocialUtilsManager.parseContent(event.content);
                    stats = {
        ...
        async generateGroupKeys(privateKey, encryptionPublicKeys) {
            ...
            for (let encryptionPublicKey of encryptionPublicKeys) {
                const encryptedGroupKey = await utilsManager_4.SocialUtilsManager.encryptMessage(privateKey, encryptionPublicKey, groupPrivateKey);
        ...
        async createCommunity(newInfo, creatorId) {
            const communityUri = utilsManager_4.SocialUtilsManager.getCommunityUri(creatorId, newInfo.name);
            let communityInfo = {
                communityUri,
        ...
        async updateCommunity(info) {
            if (info.membershipType === interfaces_4.MembershipType.NFTExclusive) {
                ...
                let encryptedGroupKeys = {};
                for (let encryptionPublicKey of encryptionPublicKeys) {
                    const encryptedGroupKey = await utilsManager_4.SocialUtilsManager.encryptMessage(this._privateKey, encryptionPublicKey, groupPrivateKey);
                    encryptedGroupKeys[encryptionPublicKey] = encryptedGroupKey;
                }
        ...
        async encryptGroupMessage(privateKey, groupPublicKey, message) {
            ...
            const encryptedGroupKey = await utilsManager_4.SocialUtilsManager.encryptMessage(privateKey, groupPublicKey, messagePrivateKey);
            const encryptedMessage = await utilsManager_4.SocialUtilsManager.encryptMessage(privateKey, messagePublicKey, message);
            return {
                encryptedMessage,
                encryptedGroupKey
            };
        ...
        async retrieveChannelEvents(creatorId, channelId) {
            ...
            if (channelMetadataEvent) {
                channelInfo = utilsManager_4.SocialUtilsManager.extractChannelInfo(channelMetadataEvent);
            }
            else {
                channelInfo = utilsManager_4.SocialUtilsManager.extractChannelInfo(channelCreationEvent);
            }
            if (!channelInfo)
                throw new Error('No info event found');
            return {
        ...
        async retrieveChannelMessageKeys(options) {
            ...
            if (options.gatekeeperUrl) {
                ...
                if (channelInfo.scpData.communityUri) {
                    const { communityId } = utilsManager_4.SocialUtilsManager.getCommunityBasicInfoFromUri(channelInfo.scpData.communityUri);
                    ...
                    if (keyEvent) {
                        ...
                        const pubkey = utilsManager_4.SocialUtilsManager.convertPrivateKeyToPubkey(options.privateKey);
                        const memberKeyMap = JSON.parse(keyEvent.content);
                        const encryptedKey = memberKeyMap?.[pubkey];
                        if (encryptedKey) {
                            groupPrivateKey = await utilsManager_4.SocialUtilsManager.decryptMessage(options.privateKey, creatorPubkey, encryptedKey);
                        }
                    }
                }
            ...
            for (let event of generalMembersEvents) {
                ...
                for (let communityUri of communityUriArr) {
                    if (!communityUriToMemberIdRoleComboMap[communityUri])
                        continue;
                    const pubkey = index_6.Nip19.npubEncode(event.pubkey);
        ...
        async updateCalendarEvent(updateCalendarEventInfo) {
            ...
            if (response.success) {
                const pubkey = utilsManager_4.SocialUtilsManager.convertPrivateKeyToPubkey(this._privateKey);
        ...
        async fetchTimezones() {
            ...
            for (let timezone of apiResult.data.timezones) {
                let gmtOffset = utilsManager_4.SocialUtilsManager.getGMTOffset(timezone.timezoneName);
                if (!gmtOffset)
                    continue;
                timezones.push({
        ...
        async fetchInstalledByPubKey(pubkey) {
            ...
            if (!installed)
                return null;
            const decrypted = await (0, crypto_2.decryptWithPrivKey)(this._privateKey, installed);
        ...
        async installApp(pubkey, appId, appVersionId) {
            ...
            const encryptedInstalledAppList = await (0, crypto_2.encryptWithPubKey)(pubkey, JSON.stringify(newInstalledApps));
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
...
define("@scom/scom-social-sdk", ["require", "exports", "@scom/scom-social-sdk/core/index.ts", "@scom/scom-social-sdk/utils/index.ts", "@scom/scom-social-sdk/managers/index.ts"], function (require, exports, index_7, index_8, managers_1) {
    ...
    Object.defineProperty(exports, "MqttManager", { enumerable: true, get: function () { return index_8.MqttManager; } });
