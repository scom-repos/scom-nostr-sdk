...
class SocialDataManager {
    ...
    async constructCommunityNoteIdToPrivateKeyMap(communityInfo, noteInfoList) {
        ...
        for (const note of noteInfoList) {
            let postScpData = utilsManager_1.SocialUtilsManager.extractScpData(note.eventData, interfaces_1.ScpStandardId.CommunityPost);
            if (!postScpData)
                continue;
            let postPrivateKey;
            if (note.eventData.pubkey === pubkey) {
                postPrivateKey = await utilsManager_1.SocialUtilsManager.decryptMessage(this._privateKey, communityInfo.scpData.publicKey, postScpData.encryptedKey);
            }
            else {
    ...
    async retrieveCommunityThreadPostKeys(options) {
        ...
        if (policyTypes.includes(interfaces_1.ProtectedMembershipPolicyType.TokenExclusive) && options.gatekeeperUrl) {
            let bodyData = utilsManager_1.SocialUtilsManager.augmentWithAuthInfo({
                creatorId: communityInfo.creatorId,
                communityId: communityInfo.communityId,
    ...
    async retrieveCommunityPostKeysByNoteEvents(options) {
        ...
        for (let communityInfo of noteCommunityMappings.communityInfoList) {
            const policyTypes = communityInfo.policies?.map(v => v.policyType) || [];
            if (options.pubKey === communityInfo.creatorId || policyTypes.includes(interfaces_1.ProtectedMembershipPolicyType.Whitelist)) {
                let communityPrivateKey = await this.retrieveCommunityPrivateKey(communityInfo, this._privateKey);
                if (communityPrivateKey) {
            ...
            if (communityPrivateKey) {
                ...
                if (communityInfo.privateRelay) {
                    ...
                    for (let policy of communityInfo.policies) {
                        if (policy.policyType === interfaces_1.ProtectedMembershipPolicyType.Whitelist) {
                        ...
                        else if (policy.policyType === interfaces_1.ProtectedMembershipPolicyType.TokenExclusive) {
    ...
    async createCommunity(newInfo, creatorId) {
        ...
        if (communityInfo.membershipType === interfaces_1.MembershipType.Protected) {
            ...
            const creatorPubkey = index_1.Nip19.decode(communityInfo.creatorId).data;
            ...
            for (let policy of communityInfo.policies) {
                if (policy.policyType === interfaces_1.ProtectedMembershipPolicyType.TokenExclusive) {
                }
                else if (policy.policyType === interfaces_1.ProtectedMembershipPolicyType.Whitelist) {
    ...
    async updateCommunity(info) {
        if (info.membershipType === interfaces_1.MembershipType.Protected) {
            ...
            const creatorPubkey = index_1.Nip19.decode(info.creatorId).data;
            let encryptionPublicKeys = [creatorPubkey, gatekeeperPublicKey];
            ...
            for (let policy of info.policies) {
                if (policy.policyType === interfaces_1.ProtectedMembershipPolicyType.TokenExclusive) {
                    ...
                    for (let memberId of policy.memberIds) {
                        const memberPublicKey = memberId.startsWith('npub1') ? index_1.Nip19.decode(memberId).data : memberId;
                        encryptionPublicKeys.push(memberPublicKey);
                    }
                    memberIds = [...memberIds, ...policy.memberIds];
                }
