...
class SocialDataManager {
    ...
    async fetchInstalledByPubKey(pubkey) {
        ...
        const decrypted = await utilsManager_1.SocialUtilsManager.decryptMessage(this._privateKey, pubkey, installed);
    ...
    async installApp(pubkey, appId, appVersionId) {
        ...
        const encryptedInstalledAppList = await utilsManager_1.SocialUtilsManager.encryptMessage(this._privateKey, pubkey, JSON.stringify(newInstalledApps));
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
