...
class SocialDataManager {
    ...
    async fetchInstalledByPubKey(pubkey) {
        ...
        if (!installed)
            return null;
        const decrypted = await (0, crypto_1.decryptWithPrivKey)(this._privateKey, installed);
    ...
    async installApp(pubkey, appId, appVersionId) {
        ...
        const encryptedInstalledAppList = await (0, crypto_1.encryptWithPubKey)(pubkey, JSON.stringify(newInstalledApps));
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
