...
class SocialUtilsManager {
    ...
    static extractCommunityInfo(event) {
        ...
        let scpData;
        let gatekeeperNpub;
        let membershipType = utils_1.MembershipType.Open;
        ...
        if (scpTag && scpTag[1] === '1') {
            membershipType = utils_1.MembershipType.Protected;
                ...
                if (data.policies) {
                    for (let policy of data.policies) {
                        ...
                        if (policy.paymentMethod) {
                            ...
                            paymentMethod = utils_1.PaymentMethod.EVM;
                        }
                        else {
                            paymentMethod = policy.currency === 'TON' ? utils_1.PaymentMethod.TON : utils_1.PaymentMethod.Telegram;
                        }
                        policies.push({
                            ...policy,
                            paymentMethod
    ...
    static extractCommunityProductInfo(event) {
        ...
        let communityProductInfo = {
            ...
            productType: data.product_type || utils_1.MarketplaceProductType.Physical,
            name: data.name,
            description: data.description,
            images: data.images,
            currency: data.currency,
    ...
    static extractChannelInfo(event) {
        ...
        if (!eventId)
            return null;
        let scpData = this.extractScpData(event, utils_1.ScpStandardId.Channel);
        let channelInfo = {
            id: eventId,
            name: content.name,
            about: content.about,
