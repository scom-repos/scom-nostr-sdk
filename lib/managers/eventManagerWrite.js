...
class NostrEventManagerWrite {
    ...
    async placeMarketplaceOrder(options) {
        ...
        const encryptedMessage = await utilsManager_1.SocialUtilsManager.encryptMessage(this._privateKey, decodedMerchantPubkey, JSON.stringify(message));
        let event = {
            "kind": 4,
            "created_at": Math.round(Date.now() / 1000),
            "content": encryptedMessage,
    ...
    async recordPaymentActivity(options) {
        ...
        if (stallId) {
            const stallUri = utilsManager_1.SocialUtilsManager.getMarketplaceStallUri(recipient, stallId);
            event.tags.push([
                "a",
                stallUri
            ]);
