...
class NostrEventManagerWrite {
    ...
    async updateCommunity(info) {
        ...
        if (info.scpData) {
            ...
            event.tags.push([
                "scp",
                utils_1.ScpStandardId.Community,
                encodedScpData
            ]);
        }
    ...
    async submitCommunityPost(info) {
        ...
        if (info.scpData) {
            ...
            event.tags.push([
                "scp",
                utils_1.ScpStandardId.CommunityPost,
                encodedScpData
            ]);
        }
    ...
    async submitChannelMessage(info) {
        ...
        if (info.scpData) {
            ...
            event.tags.push([
                "scp",
                utils_1.ScpStandardId.ChannelMessage,
                encodedScpData
            ]);
        }
        if (info.conversationPath) {
    ...
    async updateGroupKeys(identifier, groupKind, keys, invitees) {
        let event = {
            ...
            "tags": [
                ...
                [
                    "scp",
                    utils_1.ScpStandardId.GroupKeys
                ]
            ]
        };
    ...
    async updateCalendarEvent(info) {
        ...
        let start;
        let end;
        if (info.type === utils_1.CalendarEventType.DateBased) {
            kind = 31922;
    ...
    async updateCommunityProduct(creatorId, communityId, product) {
        ...
        const productContent = JSON.stringify({
            ...
            product_type: product.productType || utils_1.MarketplaceProductType.Physical,
            name: product.name,
