...
class NostrEventManagerReadV1o5 {
    ...
    async getCommunityUriToMembersMap(communities) {
        ...
        for (let event of communityMemberEvents) {
            ...
            if (!creatorProfile)
                continue;
            let creator = mapProfileToCommunityMember(creatorProfile, utils_1.CommunityRole.Creator);
            communityMembers.push(creator);
            for (let moderator of content.moderators) {
                const userProfile = pubkeyToProfileMap[moderator];
                if (!userProfile)
                    continue;
                let communityMember = mapProfileToCommunityMember(userProfile, utils_1.CommunityRole.Moderator);
            ...
            for (let member of content.general_members) {
                ...
                if (!userProfile)
                    continue;
                let communityMember = mapProfileToCommunityMember(userProfile, utils_1.CommunityRole.GeneralMember);
                communityMembers.push(communityMember);
            }
