...
class SocialDataManager {
    ...
    async retrievePostPrivateKey(event, communityUri, communityPrivateKey) {
        let key = null;
        let postScpData = utilsManager_1.SocialUtilsManager.extractScpData(event, utils_1.ScpStandardId.CommunityPost);
        if (!postScpData)
            return key;
        try {
    ...
    async retrieveChannelMessagePrivateKey(event, channelId, communityPrivateKey) {
        let key = null;
        let messageScpData = utilsManager_1.SocialUtilsManager.extractScpData(event, utils_1.ScpStandardId.ChannelMessage);
        try {
    ...
    async retrieveCommunityPostKeysByNoteEvents(options) {
        ...
        for (let communityInfo of noteCommunityMappings.communityInfoList) {
            ...
            if (communityInfo.membershipType === utils_1.MembershipType.Open)
                continue;
            let communityPrivateKey = await this.retrieveCommunityPrivateKey(communityInfo, this._privateKey);
    ...
    constructNoteCommunity(noteEvent, communityInfoMap) {
        ...
        if (noteEvent.tags?.length) {
            let scpData = utilsManager_1.SocialUtilsManager.extractScpData(noteEvent, utils_1.ScpStandardId.CommunityPost);
            ...
            if (communityUri) {
                ...
                community = {
                    ...
                    isExclusive: communityInfo?.membershipType === utils_1.MembershipType.Protected,
                    isWhitelist: communityInfo?.policies?.[0]?.policyType === utils_1.ProtectedMembershipPolicyType.Whitelist,
    ...
    async fetchCommunityInfo(creatorId, communityId) {
        ...
        if (communityInfo.membershipType === utils_1.MembershipType.Protected && !communityInfo.scpData?.encryptedKey) {
    ...
    async fetchThreadNotesInfo(focusedNoteId) {
        ...
        let scpData = utilsManager_1.SocialUtilsManager.extractScpData(focusedNote.eventData, utils_1.ScpStandardId.CommunityPost);
        if (scpData) {
    ...
    async createNoteCommunityMappings(notes) {
        ...
        for (let note of notes) {
            let scpData = utilsManager_1.SocialUtilsManager.extractScpData(note, utils_1.ScpStandardId.CommunityPost);
            if (scpData) {
    ...
    async createCommunity(newInfo, creatorId) {
        ...
        if (communityInfo.membershipType === utils_1.MembershipType.Protected) {
    ...
    async updateCommunity(info) {
        if (info.membershipType === utils_1.MembershipType.Protected) {
    ...
    async fetchUserRoleInCommunity(community, pubKey) {
        if (!pubKey)
            return utils_1.CommunityRole.None;
        if (community.creatorId === pubKey)
            return utils_1.CommunityRole.Creator;
        if (community.moderatorIds?.includes(pubKey))
            return utils_1.CommunityRole.Moderator;
        ...
        return isMember ? utils_1.CommunityRole.GeneralMember : utils_1.CommunityRole.None;
    }
    ...
    async submitCommunityPost(message, info, conversationPath, timestamp, alt, isPublicPost = false) {
        ...
        if (info.membershipType === utils_1.MembershipType.Open || isPublicPost) {
    ...
    extractCalendarEventInfo(event) {
        ...
        let endTime;
        if (event.kind === 31922) {
            type = utils_1.CalendarEventType.DateBased;
            ...
            type = utils_1.CalendarEventType.TimeBased;
    ...
    async updateCalendarEvent(updateCalendarEventInfo) {
        ...
        if (response.success) {
            naddr = index_1.Nip19.naddrEncode({
                ...
                pubkey: this.selfPubkey,
                kind: updateCalendarEventInfo.type === utils_1.CalendarEventType.DateBased ? 31922 : 31923,
                relays: []
            });
        }
        return naddr;
