...
class SocialDataManager {
    ...
    async fetchCommunityOrders(creatorId, communityId, stallId, status) {
        ...
        for (let event of orderEvents) {
            const order = await utilsManager_1.SocialUtilsManager.extractMarketplaceOrder(this._privateKey, event);
            if (!order)
                continue;
    ...
    async fetchMarketplaceOrderDetails(orderId) {
        ...
        const order = await utilsManager_1.SocialUtilsManager.extractMarketplaceOrder(this._privateKey, orderEvent);
    ...
    async fetchProductPostPurchaseContent(sellerPubkey, productId, postPurchaseContent) {
        ...
        const text = await utilsManager_1.SocialUtilsManager.decryptMessage(contentKey, sellerPubkey, postPurchaseContent);
        return text;
    }
    async fetchRegions() {
